...
\section{はじめに}
  ...
  上記の問題点を踏まえ、特定の波長の光を切り出すデバイスであるOptical Filter(OF)の波長を任意に変更することで出力としてXOR回路としてだけでなくAND回路としても使用でき,半加算器として使用する.
...
\section{結果}
  % \begin{wrapfigure}[55]{r}{0pt}{0.5\textwidth}
  \begin{wrapfigure}{r}[0pt]{0.6\textwidth}
    \centering
    \includegraphics[clip,width=0.4\columnwidth]{images/input_output.png}
    \caption{提案デバイスの入出力波形}
    \begin{tabular}{cc}
      \begin{minipage}[t]{0.5\hsize}
        \centering
        \includegraphics[clip,width=0.6\columnwidth]{images/AND_RESULT.png}
        \caption{AND演算の\\アイダイアグラム}
      \end{minipage} &
      \begin{minipage}[t]{0.5\hsize}
        \centering
        \includegraphics[clip,width=0.6\columnwidth]{images/XOR_RESULT.png}
